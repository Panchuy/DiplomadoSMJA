...
float distancia(int A,int B){
   return sqrt(pow(nodes[A].x-nodes[B].x,2)+pow(nodes[A].y-nodes[B].y,2));
}
		...
		for(int j=0 ; j < nodes[D].num_conections; j++  )
   		{
   			if(nodes[nodes[D].conections[j].node].flag == 'N')
   				{
   					nodes[nodes[D].conections[j].node].acumulado = nodes[D].acumulado + nodes[D].conections[j].cost + distancia(nodes[D].conections[j].node,L);
   			...
   			{
   				if( nodes[nodes[D].conections[j].node].acumulado > nodes[D].acumulado + nodes[D].conections[j].cost+distancia(nodes[D].conections[j].node,L))
   				{
   					nodes[nodes[D].conections[j].node].acumulado = nodes[D].acumulado + nodes[D].conections[j].cost+distancia(nodes[D].conections[j].node,L);
   					nodes[nodes[D].conections[j].node].parent = D;
   				}
...
int a_star(float rx ,float ry ,float lx ,float ly, char *world_name,step *steps )
{
   char archivo[150];
   int i;
   int start = 0;
...
*/
   a_star_algorithm (goal ,start); // se pasan alreves para no tener que voltear la lista resultante
   int padre = start;
